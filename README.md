# go学习笔记（此篇是在郝琳老师的go36讲的基础上结合自己的学习做的总结，借用了老师的目录结构）
...
1.net.Dial(network,address)函数的第一个参数network有哪些可选值？
  
  1.tcp,tcp4,tcp6,udp,udp4,udp6,unix,uinxgram,unixpacket
  
2.syscall.socket函数的三个参数都是int类型，分别代表了想要创建的socket实例通信域，类型，以及使用协议

通信域的三种选择：ipv4域，ipv6域，unix域

3.调用net.DialTimeout函数给定的超时时间意味着什么？

简单来说，这里的超时时间，代表着这个函数为网络链接建立完成而等待的最长时间。这是一个相对时间。他会有这个函数的参数timeout的值表示。在绝对的超时时间到达的那一刻，网络链接还没有建立完成，该函数就会返回一个代表了i/o操作超时的错误值。

值得注意的是，在解析address的值的时候，函数会确定网络服务的ip地址，端口号等必要信息，并在需要时访问dns服务。

另外，如果解析出的ip地址有多个，那么函数就会串行或并发的尝试建立链接。但无论用什么样的方式尝试，函数总会以最先建立成功的那个连接为准。

同时它还会根据超时前的剩余时间，去设定针对每次连接尝试的超时时间，以便让他们都有适当的时间执行。

再多说一点。在net包中还有一个名为Dialer的结构体类型。该类型有一个名叫Timeout的字段，他与上述的timeout参数的含义是完全一致的。实际上，net.DialTimeout函数利用了这个类型的值才得以实现功能的。

4.当调用了net.Dial等函数之后，如果成功获得了一个代表网络连接的net.conn接口类型的值。可以调用setdeadlinelmpl(fd,t,'t'+'w')对读和写都设置了超时时间。

# 四十七.基于http协议的网络服务

1.比如说使用http.get函数会发送一个get请求，第一个结果值类型是×http.Response,它是网络服务给我们传回来的响应内容的结构化表示
第二个值error类型的，它代表了创建在和发送http请求，以及接收和解析http
响应的过程中可能发生的错误。get会在内部使用缺省http客户端，这个客户端是有net/http包中的公开变量defaultclinet代表的，其类型×http.client。它的基本类型也是可以被拿来使用的，甚至他还是开箱即用的。

2.http.client的transport字段代表着什么？
 
transport字段代表着：向网络服务发送http请求，并从网络服务接收http响应的过程。也就是说，该字段的方法roundtrip应该实现单词http事物所需要的步骤。

这个字段是http.roundtripper接口类型的,它有一个由http.DefaultTransport变量代表的缺省值，当我们初始化一个http.client类型的值的时候，如果没有显示的为该字段赋值，那么这个client值就会直接使用DefaultTransport。

空闲连接的总数最大为100，而针对每个网络服务的最大空闲连接数为2.
